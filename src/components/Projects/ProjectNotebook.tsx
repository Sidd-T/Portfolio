...
const ProjectNotebook: React.FC<NotebookProps> = ({ index, image, title, description, coverNum, setSelectedNotebook }) => {
  ...
  return (
    <div 
      className={`mt-6 mx-auto w-[30vh] sm:w-[50vh] h-[50vh] sm:h-[70vh] bg-white rounded-xl shadow-[rgba(0,0,15,0.3)_10px_-4px_4px_3px] flex flex-col pb-[50%] notebook-cover-${coverNum % 4} 
    ...
    >
      {/* Image section */}
      <div className="py-2 sm:py-4 px-6 w-full h-full rounded-xl">
      ...
      <div className="py-2 sm:py-4 px-6">
        ...
        <p className="text-xs sm:text-md text-white mt-2 overflow-auto">{description}</p>
      </div>
    </div>
  );
};
